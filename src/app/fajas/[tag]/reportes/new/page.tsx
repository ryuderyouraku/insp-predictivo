...
import { listSupervisoresDeContratista, listEspecialistasDeContratista } from '@/server/actions/users'
...
export default async function NewReportePage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  const [supervisores, especialistas] = await Promise.all([
    listSupervisoresDeContratista(faja.contratistaId),
    listEspecialistasDeContratista(faja.contratistaId),
  ])
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6">
      ...
      <ReporteForm faja={faja} currentUserId={user.id} supervisores={supervisores} especialistas={especialistas} />
    </main>
  )
}
